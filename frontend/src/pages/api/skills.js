...
export default async function handler(req, res) {
  ...
  try {
    if (method === 'GET') {
      const { data, error } = await supabaseAdmin.from('skills').select('*').order('created_at', { ascending: false });
    ...
    if (method === 'POST') {
      const body = req.body;
      const { data, error } = await supabaseAdmin.from('skills').insert([body]).select();
      if (error) throw error;
    ...
    if (method === 'PUT') {
      ...
      const { data, error } = await supabaseAdmin.from('skills').update(updates).eq('id', id).select();
      if (error) throw error;
