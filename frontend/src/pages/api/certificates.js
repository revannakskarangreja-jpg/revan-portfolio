...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (method === 'POST') {
      const body = req.body;
      const { data, error } = await supabaseAdmin.from('certificates').insert([body]).select();
      if (error) throw error;
    ...
    if (method === 'PUT') {
      ...
      const { data, error } = await supabaseAdmin.from('certificates').update(updates).eq('id', id).select();
      if (error) throw error;
