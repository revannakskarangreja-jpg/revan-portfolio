...
export default async function handler(req, res) {
  ...
  // Very small auth: expect Authorization: Bearer <access_token>
  ...
  try {
    ...
    // For write operations verify admin user
    ...
    if (method === 'PUT') {
      ...
      const { data, error } = await supabaseAdmin.from('projects').update(updates).eq('id', id).select();
      if (error) throw error;
